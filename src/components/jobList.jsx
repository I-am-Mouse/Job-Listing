...
const JobList = ({job, addToFilters}) => {
    ...
    return (
        <article className="flex">
            ...
            <div>
                <button onClick={() => 
                    addToFilters({type: "role", value: job.role})}>{job.role}
                </button>

                <button onClick={() => 
                    addToFilters({type: "level", value: job.level})}>{job.level}
                </button>

                {
                    job.languages.map((language, index) => {
                    return ( <button key={index} 
                                onClick={() => addToFilters({type: "language", value: language})}>{language}
                            </button>)})   //#1st way
                }

                {
                    job.tools.map((tool, index) => <button key={index} onClick={() => addToFilters({type: "tool", value: tool})}>{tool}</button> )  //#2nd way
                }
            </div>
        </article>
    )
